...
class Chemical_Reaction:
    ...
    def complete_reaction(self, reactant_flowrates):
        ...
        reactant_molflowrates = np.zeros(len(self.reactants))
        stoichmetric_molflowrates = np.zeros(len(self.reactants))
        ...
        for k in range(len(self.reactants)):
            ...
            reactant_flowrates[k] = float(reactant_molflowrates[k] * self.reactants[k].get_mol_weight())

        product_molflowrates = np.zeros(len(self.products))
        product_flowrates = np.zeros(len(self.products))
        for k in range(len(self.products)):
            product_molflowrates[k] = molflowrate * self.product_stoich_coeff[k]
            product_flowrates[k] = float(product_molflowrates[k] * self.products[k].get_mol_weight())

        P_total = float(self.Q_reaction * molflowrate)
